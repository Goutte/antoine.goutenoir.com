...
LeapElement.prototype = {
    ...
    dispatchEvent: function() {
      return this.element.dispatchEvent.apply(this.element, arguments)
    },
    getEventHandler: function() {
      return this.element.getEventHandler.apply(this.element, arguments)
    },
    setEventHandler: function() {
      return this.element.setEventHandler.apply(this.element, arguments)
    },
                        ...
                        if (!cursor.isCaptured()) {
                            if (cursor.attractor) {
                                ...
                                cursor.setVelocityXYZ(
                                    xDiff * Math.max(minPull, Math.min(maxPull, xRatio)),
                                    yDiff * Math.max(minPull, Math.min(maxPull, yRatio)), 0);
...
var LeapManager = (function() {
    ...
    var defaultConfig = {
        maxCursors: 1,
        enableFrameScrolling: false, // move cursor close to frame borders to scroll
        ...
        frameScrollingConfig: {
            treshold: 0.1618, // fraction of the screen that reacts, around the frame
            speed: 5
        },
    ...
    return {
        ...
        init: function(config) {
            ...
            this.enableFrameScrolling = config.enableFrameScrolling;
            this.frameScrollingConfig = config.frameScrollingConfig;
            ...
            var callback = this.loopCallback = function(frame) {
                me.onLoop(frame);
            };
        ...
        //// CUSTOM METHOD
        exit: function() {
            if(Leap !== null && Leap.loopController && this.loopCallback) {
                Leap.loopController.removeListener(Leap.loopController.frameEventName, this.loopCallback);
            }
            this.updatePointables(Leap.Frame.Invalid);
            this.updateGestures(Leap.Frame.Invalid);
            this.loopCallback = null;
            this.cursorManager.cursorContainer.element.parentNode.removeChild(this.cursorManager.cursorContainer.element);
        },
        onLoop: function(frame) {
            ...
            this.updateHand(frame);

            // DISABLE POINTABLES
            // They're too shaky, we're using the hand's palmPosition
            //this.updatePointables(frame);

            // DISABLE GESTURES
            // We're managing them sanely in Jax, yet this is loaded before Jax,
            // so the Loop's config must enable Gestures ;
            // yet we don't need those, and we need the horsepower
        ...
        //// CUSTOM METHOD
        updateHand: function(frame) {
          var currentCursors = [];

          if (frame && frame.hands && frame.hands[0]) {
            var hand = frame.hands[0];
            var palm = hand.palmPosition;
            var posX = LeapManagerUtils.map(palm[0], this.boundary.left, this.boundary.right, 0, 1);
            var posY = LeapManagerUtils.map(palm[1], this.boundary.bottom, this.boundary.top, 1, 0);
            var posZ = palm[2];
            var cursor = this.getCursor(hand.id, {
              x: posX,
              y: posY,
              z: posZ
            });
            ...
            this.scrollIfNeeded(posY);
          }

          this.cursorManager.pruneCursors(LEAP_SOURCE, currentCursors);
        },
        //// CUSTOM METHOD
        scrollIfNeeded: function(posY) {
          if (this.enableFrameScrolling) {
            if (posY > 1 - this.frameScrollingConfig.treshold) {
              window.scrollBy(0, this.frameScrollingConfig.speed);
            } else if (posY < this.frameScrollingConfig.treshold) {
              window.scrollBy(0, this.frameScrollingConfig.speed * -1);
            }
          }
        },
